...
export class ConstraintMap {
  static toConstraintList = (constraintMap: ConstraintMapObject, sortBy: ((c: Constraint) => string | number)[]): Constraint[] => {
  ...
  static fromConstraintList = (constraintList: Constraint[]): ConstraintMapObject => {
    const constraintMap = constraintList.reduce((dict, constraint) => {
